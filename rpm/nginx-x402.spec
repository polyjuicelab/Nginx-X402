        ...
        if [ -d "/tmp/nginx-$NGINX_VERSION" ]; then
            ...
            if [ -n "$NGINX_CONFIGURE_ARGS_SYSTEM" ]; then
                ...
                if [ -f "/tmp/nginx-$NGINX_VERSION/objs/ngx_auto_config.h" ]; then
                    ...
                    # Extract signature from configured nginx source for comparison
                    if [ -f "/tmp/nginx-$NGINX_VERSION/objs/ngx_modules.c" ]; then
                        echo ""
                        ...
                        if [ -n "$SOURCE_SIG" ]; then
                            ...
                            if command -v strings >/dev/null 2>&1 && [ -f "/usr/sbin/nginx" ]; then
                                ...
                                if [ -n "$NGINX_SIG" ]; then
                                    echo "System nginx signature: $NGINX_SIG"
                                    if [ "$SOURCE_SIG" = "$NGINX_SIG" ]; then
                                        echo "✓ Source signature matches system nginx signature"
                                    else
                                        echo "⚠ WARNING: Source signature does NOT match system nginx signature"
