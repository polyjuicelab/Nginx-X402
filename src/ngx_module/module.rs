...
use ngx::ffi::{ngx_http_core_loc_conf_t, ngx_http_conf_ctx_t, ngx_http_core_main_conf_t};
use std::ffi::c_char;
use std::ptr;
...
unsafe fn get_core_loc_conf(cf: *mut ngx::ffi::ngx_conf_t) -> Option<*mut ngx::ffi::ngx_http_core_loc_conf_t> {
...
unsafe fn get_core_main_conf(cf: *mut ngx::ffi::ngx_conf_t) -> Option<*mut ngx_http_core_main_conf_t> {
    if cf.is_null() {
        return None;
    }
